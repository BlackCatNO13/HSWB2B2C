    ...
    public class ShopBonusController : BaseMobileTemplatesController
    {
        ...
        public ActionResult Index(long id)
        {
            ...
            if (string.IsNullOrEmpty(item))
            {
                ...
                string authorizeUrl = Senparc.Weixin.MP.AdvancedAPIs.OAuthApi.GetAuthorizeUrl(_siteSetting.WeixinAppId.Trim(), absoluteUri, "STATE#wechat_redirect", OAuthScope.snsapi_userinfo, "code");
                return Redirect(authorizeUrl);
            }
            try
            {
                accessToken = Senparc.Weixin.MP.AdvancedAPIs.OAuthApi.GetAccessToken(_siteSetting.WeixinAppId.Trim(), _siteSetting.WeixinAppSecret.Trim(), item, "authorization_code");
                goto Label0;
            }
            ...
            Label0:
            Log.Info("------------------------------");
            OAuthUserInfo userInfo = Senparc.Weixin.MP.AdvancedAPIs.OAuthApi.GetUserInfo(accessToken.access_token, accessToken.openid, Language.zh_CN);
